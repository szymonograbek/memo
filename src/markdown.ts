...
const decodeDocument = (path: string, raw: string) => {
  ...
  return Schema.decodeUnknown(MarkdownDocument)({
    ...
    Effect.mapError((error) => new MarkdownError({ message: `${path}: ${error.message}` })),
  )
}

const decodeTypedFrontmatter = (path: string, frontmatter: unknown) =>
  Schema.decodeUnknown(FrontmatterWithType)(frontmatter).pipe(
    Effect.mapError((error) => new MarkdownError({ message: `${path}: invalid frontmatter: ${error.message}` })),
  )
...
export const decodeMarkdown = (templates: ReadonlyMap<string, TemplateDefinition>, path: string, raw: string) =>
  Effect.gen(function* () {
    ...
    if (!template) return yield* new MarkdownError({ message: `${path}: unknown template type ${typed.type}` })
...
export const encodeMarkdown = (note: MemoryNote) =>
  Schema.encode(MemoryNote)(note).pipe(
    Effect.mapError((error) => new MarkdownError({ message: `Failed to encode ${note.path}: ${error.message}` })),
    Effect.flatMap((encoded) => Effect.try({
      try: () => matter.stringify(encoded.body, encoded.frontmatter),
      catch: (error) => new MarkdownError({ message: `Failed to encode ${note.path}: ${String(error)}` }),
    })),
  )
