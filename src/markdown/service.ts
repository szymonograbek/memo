  ...
  Schema.decodeUnknown(FrontmatterJson)(input).pipe(
    Effect.mapError(
      (error) =>
        new MarkdownError({
          reason: "InvalidJson",
          message: `Invalid frontmatter JSON: ${error.message}`,
        }),
    ),
  )
...
const decodeDocument = (path: string, raw: string) => {
  ...
  return Schema.decodeUnknown(MarkdownDocument)({
    ...
    Effect.mapError(
      (error) =>
        new MarkdownError({ reason: "InvalidFrontmatter", message: `${path}: ${error.message}` }),
    ),
  )
}
    ...
    Effect.mapError(
      (error) =>
        new MarkdownError({
          ...
          message: `${path}: invalid frontmatter: ${error.message}`,
        }),
    ),
  )
...
export const normalizeFrontmatter = (frontmatter: Readonly<Record<string, unknown>>) => {
  ...
  return {
    ...
    updatedAt:
      asDateString(frontmatter.updatedAt) ?? asDateString(frontmatter.updated) ?? fallbackDate,
...
export const decodeMarkdown = Effect.fnUntraced(function* (
  ...
  if (!template) {
    return yield* new MarkdownError({
      ...
      message: `${path}: unknown template type ${typed.type}`,
    })
  }
  ...
  Schema.encode(MemoryNote)(note).pipe(
    Effect.mapError(
      (error) =>
        new MarkdownError({
          reason: "EncodeFailed",
          message: `Failed to encode ${note.path}: ${error.message}`,
        }),
    ),
    Effect.flatMap((encoded) =>
      Effect.try({
        try: () => matter.stringify(encoded.body, encoded.frontmatter),
          ...
          new MarkdownError({
            reason: "EncodeFailed",
            message: `Failed to encode ${note.path}: ${String(error)}`,
          }),
      }),
    ),
  )
