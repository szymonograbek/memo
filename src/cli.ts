...
const withMemory = Effect.fnUntraced(function* <A, E, R>(
  ...
  return yield* effect.pipe(
    Effect.provide(
      MemoryService.layer({ rootDir: config.rootDir, templateDirs: [config.templateDir] }),
    ),
  )
})
    ...
    withMemory(
      Effect.gen(function* () {
        ...
        const filtered =
          typeValue === undefined
            ? notes
            : notes.filter((note) => note.frontmatter.type === typeValue)
...
const links = Command.make(
  "links",
  { path: Args.path({ name: "path" }).pipe(Args.optional) },
  ({ path }) =>
    withMemory(
      Effect.gen(function* () {
        const memory = yield* MemoryService
    ...
    withMemory(
      Effect.gen(function* () {
        ...
        yield* Option.match(saveBodyTo, {
          onNone: () => Effect.void,
          onSome: (file) =>
            fs.writeFileString(file, note.body).pipe(
              Effect.catchTags({
...
const patch = Command.make(
  ...
  ({ path, frontmatter, body, bodyFile }) =>
    withMemory(
      Effect.gen(function* () {
        const memory = yield* MemoryService
        const fs = yield* FileSystem.FileSystem
        ...
        if (Option.isNone(frontmatter) && bodyValue === undefined) {
          return yield* new MemoryError({
            reason: "ConflictingArguments",
            message: "At least one of --frontmatter, --body, or --body-file must be provided",
          })
        }
...
const create = Command.make(
  ...
  ({ type, frontmatter, body, bodyFile }) =>
    withMemory(
      Effect.gen(function* () {
        const memory = yield* MemoryService
        const fs = yield* FileSystem.FileSystem
...
const command = Command.make("memo").pipe(
