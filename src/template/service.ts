...
const decodeField = (template: TemplateDefinition, key: string, field: FieldSpec, value: unknown) =>
  Schema.decodeUnknown(fieldSchema(field))(value).pipe(
    Effect.asVoid,
    Effect.mapError((error) => new TemplateError({ reason: "ValidationFailed", message: `${template.type}.${key}: ${error.message}` })),
  )
...
export const interpolate = Effect.fnUntraced(function* (
  ...
  if (missing.length > 0) {
    return yield* new TemplateError({ reason: "MissingPlaceholder", message: `Missing placeholder(s): ${missing.join(", ")}` })
  }
  return result
})
...
const loadTemplateFile = (fs: FileSystem.FileSystem, path: string) =>
  fs.readFileString(path, "utf8").pipe(
    Effect.mapError((error) => new TemplateError({ reason: "ReadFailed", message: `Failed to read ${path}: ${String(error)}` })),
    Effect.flatMap((content) => Effect.try({
      try: () => parse(content),
      catch: (error) => new TemplateError({ reason: "InvalidTemplateFile", message: `Failed to parse ${path}: ${String(error)}` }),
    })),
    Effect.flatMap((raw) => Schema.decodeUnknown(TemplateDefinition)(raw).pipe(
      Effect.mapError((error) => new TemplateError({ reason: "InvalidTemplateFile", message: `${path}: ${error.message}` })),
    )),
  )
...
export const loadTemplates = Effect.fnUntraced(function* (dirs: ReadonlyArray<string>) {
  ...
  for (const dir of dirs) {
    const names = yield* fs.readDirectory(dir).pipe(
      Effect.mapError((error) => new TemplateError({ reason: "ReadFailed", message: `Failed to list ${dir}: ${String(error)}` })),
    )
