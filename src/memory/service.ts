...
const listMdFiles: (fs: FileSystem.FileSystem, dir: string) => Effect.Effect<ReadonlyArray<string>> =
  Effect.fnUntraced(function* (fs: FileSystem.FileSystem, dir: string) {
...
export class MemoryService extends Context.Tag("@memory/MemoryService")<MemoryService, {
  ...
  readonly find: (query: string, limit: number, offset: number, type: string | undefined) => Effect.Effect<ReadonlyArray<SearchResult>, Errors>
  readonly latest: (type: string | undefined, limit: number, offset: number) => Effect.Effect<ReadonlyArray<MemoryNote>, Errors>
  readonly query: (field: string, value: string, limit: number, offset: number, type: string | undefined) => Effect.Effect<ReadonlyArray<MemoryNote>, Errors>
  readonly values: (field: string, type: string | undefined) => Effect.Effect<ReadonlyArray<{ readonly value: string; readonly count: number }>, Errors>
  ...
  readonly patch: (path: string, frontmatterPatch: Frontmatter | undefined, body: string | undefined) => Effect.Effect<MemoryNote, Errors>
  readonly create: (type: string, frontmatter: Frontmatter, body: string | undefined) => Effect.Effect<MemoryNote, Errors>
}>() {
  static readonly layer = (options: MemoryOptions): Layer.Layer<MemoryService, TemplateError, FileSystem.FileSystem> => Layer.effect(
    MemoryService,
    Effect.gen(function* () {
      ...
      const find = Effect.fn("MemoryService.find")(function* (query: string, limit: number, offset: number, type: string | undefined) {
        const notes = yield* list()
        const filtered = type === undefined ? notes : Arr.filter(notes, (note) => note.frontmatter.type === type)
      ...
      const query = Effect.fn("MemoryService.query")(function* (field: string, value: string, limit: number, offset: number, type: string | undefined) {
        const notes = yield* list()
      ...
      const values = Effect.fn("MemoryService.values")(function* (field: string, type: string | undefined) {
        const notes = yield* list()
      ...
      const links = Effect.fn("MemoryService.links")(function* (path: string | undefined) {
        ...
        for (const note of notes) {
          ...
          for (const raw of extractLinks(note.body)) {
            ...
            if (isResolved(resolved)) {
              ...
              unresolved = Arr.append(unresolved, new UnresolvedLink({
                from,
                raw,
        ...
        if (path !== undefined) {
          ...
          if (isUnresolved(resolved)) return yield* new MemoryError({ reason: "NoteNotFound", message: `No note matches ${path}` })
          if (isAmbiguous(resolved)) return yield* new MemoryError({ reason: "AmbiguousNote", message: `Ambiguous note ${path}: ${resolved.candidates.join(", ")}` })
      ...
      const patch = Effect.fn("MemoryService.patch")(function* (path: string, frontmatterPatch: Frontmatter | undefined, body: string | undefined) {
        const fullPath = join(options.rootDir, path)
        const note = yield* readNote(fullPath)
        ...
        if (!template) return yield* new MemoryError({ reason: "UnknownTemplate", message: `${path}: unknown template type ${String(nextFrontmatter.type)}` })
        yield* validateFrontmatter(template, nextFrontmatter).pipe(
          Effect.mapError((error) => new MemoryError({ reason: "ValidationFailed", message: `${path}: ${error.message}` })),
        )
      ...
      const create = Effect.fn("MemoryService.create")(function* (type: string, frontmatterInput: Frontmatter, body: string | undefined) {
        const template = templates.get(type)
        if (!template) return yield* new MemoryError({ reason: "UnknownTemplate", message: `Unknown template type: ${type}` })
        ...
        const relativePath = yield* interpolate(template.path.pattern, frontmatter).pipe(
          Effect.mapError((error) => new MemoryError({ reason: "ValidationFailed", message: `${template.path.pattern}: ${error.message}` })),
        )
        if (relativePath.split("/").includes("..")) {
          return yield* new MemoryError({ reason: "UnsafePath", message: `Path pattern produced unsafe path: ${relativePath}` })
        }
        const fullPath = join(options.rootDir, relativePath)
        const exists = yield* dieOnFsError(fs.exists(fullPath))
        if (exists) return yield* new MemoryError({ reason: "NoteAlreadyExists", message: `Note already exists: ${relativePath}` })
        yield* validateFrontmatter(template, frontmatter).pipe(
          Effect.mapError((error) => new MemoryError({ reason: "ValidationFailed", message: `${relativePath}: ${error.message}` })),
        )
