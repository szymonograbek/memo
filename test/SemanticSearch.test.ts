...
describe("SemanticSearch.search", () => {
  test("returns the semantically-closest note", async () => {
    ...
    ws.writeNote("notes/dog.md", noteFile("dog", "Walk the dog every morning before 8am"))
...
`,
    ...
    ws.writeNote("notes/n.md", noteFile("n", "matching content for the search test"))
    ws.writeFile(
      "memory/books/b.md",
      `---
type: book
...
`,
  ...
  test("returns scores in descending order (highest similarity first)", async () => {
    ...
    ws.writeNote("notes/b.md", noteFile("b", "Espresso depends on grind size and water pressure"))
    ws.writeNote("notes/c.md", noteFile("c", "Hiking the Tatras is best in late summer"))
